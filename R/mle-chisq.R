...
#' Get chi squared value from MLE for custom mle function
#'
...
custom_chisq <- function(custom_mle) {
  # Compute chisq statistic of fit of data with distribution with parameters param_list
  #
  # Args - custom_mle containing:
  #   data - observed data
  #   distribution - expected distribution to test against
  #   param_list - list of parameters for distribution;
  #
  # Returns :
  #   chisq statistic with number of bins 1/4*length(data)^{3/5} - corresponding to average bin size used in fitdistrplus package
  #
  # Process :
  #   create bins of equal expected values
  ...
  exp <- rep(length(data)/bin_count, bin_count)
  #If NAs given in exp, extend exp to appropriate length
  if (length(exp) < length(summary(obs))) {
    exp <- c(exp, rep(0, length(summary(obs))-length(exp)))
  }
  return (sum((summary(obs)-exp)^2/exp))
}
